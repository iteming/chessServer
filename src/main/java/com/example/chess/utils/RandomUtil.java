...
public class RandomUtil extends Random {
    ...
    public static Set<String> createSetData(int strLength, int maxCount) {

        Set<String> list = new HashSet<>();
        while (list.size() < maxCount) {
            String string = RandomUtil.generateString(strLength);
            list.add(string);
        }
        return list;
    }
    ...
    public static void main(String[] args) {
        ...
        Set<String> data = RandomUtil.createSetData(8, 100000);
        AtomicInteger index = new AtomicInteger();
        data.forEach(it -> {
            index.getAndIncrement();
            System.out.println(index.get() + ":::" + it);
        });
